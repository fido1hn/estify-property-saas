// Edge Function: redeem-tenant-invite
// Purpose:
...
Deno.serve(async (req) => {
  ...
  if (!tenantRow) {
    const { error: tenantError } = await supabase
      .from("tenants")
      .insert({ id: user_id, status: "pending" });
  ...
  if (!roleRow) {
    const { error: roleError } = await supabase
      .from("user_roles")
      .insert({ id: user_id, user_role: "tenant" });

    if (roleError) {
      return new Response(
        JSON.stringify({ error: "Failed to assign tenant role" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
